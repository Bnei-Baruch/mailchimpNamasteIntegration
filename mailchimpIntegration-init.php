 ...
 * Description: Добавляет чекбокс "Подписка на новости" в форму регистрации
 * Version: 0.2
 * Author: CasePress
...
add_action ( 'bp_core_activated_user', 'mailChimpInt_addToMailChimp', 10, 1 );

add_action ( 'delete_user', 'UnsubscribeMailChimp' );

//add_action ( 'namaste_enrolled_course', 'UpdateUserOnMailChimp', 99, 3 );
add_action ( 'publish_namaste_course', 'AddNewCourseToMailChimp', 10, 2 );

function mailChimpInt_addToMailChimp($usermeta) {
	$user = get_user_by("id", $user_id);
	UserProfile_SetDefaultFieldes ( null, $usermeta ["fieldListBP"], $user_id );
